...
def createSensorList(config):
    ...
    for entry in config.keys():
        ...
        if (type == 1):
            type = 'volt'
            sensorList[id].update({'name': 'Voltage: ' + config[entry][3]})
            if (config[entry][3] == 'PICO INTERNAL'):
                elementSize = 6
        if (type == 2):
            type = 'current'
            sensorList[id].update({'name': 'Current: ' + config[entry][3]})
            elementSize = 2
        if (type == 3):
            type = 'thermometer'
            sensorList[id].update({'name': 'Temperature: ' + config[entry][3]})
        if (type == 5):
            type = 'barometer'
            sensorList[id].update({'name': 'Barometer: ' + config[entry][3]})
            elementSize = 2
        if (type == 6):
            type = 'ohm'
            sensorList[id].update({'name': 'Ohm: ' + config[entry][3]})
        if (type == 8):
            type = 'tank'
            sensorList[id].update({'name': 'Tank: ' + config[entry][3]})
        ...
        if (type == 9):
            type = 'battery'
            sensorList[id].update({'name': 'Battery: ' + config[entry][3]})
...
client = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  # Setup UDP broadcasting listener
...
def readIncline(sensorId, elementId):
    ...
    sensorListTmp[sensorId].update({inclinometer_type: element[elementId][1] / 1000.0})
...
while True:
    ...
    # Filter and print device name and value
   # output = {}
   # for sensorId, sensorData in sensorListTmp.items():
   #     name = sensorData.get('name')
    #    value = sensorData.get('voltage') or sensorData.get('pressure') or sensorData.get('temperature') or sensorData.get('current') or sensorData.get('capacity.remaining') or sensorData.get('currentLevel') or sensorData.get('remainingCapacity') or sensorData.get('percentage') or sensorData.get('pitch') or sensorData.get('roll')
    #    if name and value is not None and '[' not in name:
     #       output[name] = value
...
for sensorId, sensorData in sensorListTmp.items():
    ...
    if name and filtered_values and '[' not in name:
        if sensorData['type'] == 'barometer':
            ...
            output["inclinometer"].update(filtered_values)
            ...
            output["battery"][name] = [
                sensorData.get('capacity.nominal'),
                filtered_values.get('capacity_remaining'),
                filtered_values.get('current'),
                filtered_values.get('voltage')
            ]
