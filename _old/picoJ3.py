...
def HexToByte(hexStr):
    ...
    """
    bytes = bytearray.fromhex(hexStr.replace(' ', ''))
    return bytes
...
def ByteToHex(byteStr):
    ...
    """
    return ' '.join([f"{x:02X}" for x in byteStr])
...
def getNextField(response):
    ...
    if (field_type == 4):  # Text string
        ...
        while (nextHex != '00'):
            ...
            response = response[3:]
            nextHex = response[0:2]
        word = HexToByte(word).decode('utf-8')
...
def readIncline(sensorId, elementId):
    sensorListTmp[sensorId].update({'pitch': element[elementId][0] / 100})
    sensorListTmp[sensorId].update({'roll': element[elementId][1] / 100})
